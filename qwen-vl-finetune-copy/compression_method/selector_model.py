...
# --- Start of STE (Straight-Through Estimator) TopK implementation ---
class STETopK(Function):
    """
    Straight-Through Estimator for TopK token selection.
    Forward:  hard topk → binary 0/1 mask
    Backward: identity (gradients pass through directly to scores)
    """
    @staticmethod
    def forward(ctx, scores, k):
        # Hard topk: select top-k tokens, binary mask
    ...
    def backward(ctx, grad_output):
        # STE: pass gradient through directly
        return grad_output, None

ste_topk = STETopK.apply
# --- End of STE TopK implementation ---
...
def qwen25vl_vision_tower_forward_selector(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
    ...
    # STE TopK: forward uses hard binary mask, backward passes gradients through
    img_mask = ste_topk(learned_scores.unsqueeze(0), k).squeeze(0)
    ...
    # -------------------------------------------------------------------------------
    return hidden_states_new, img_mask
...
def qwen25vl_generation_forward_selector(
    ...
    if inputs_embeds is None:
        ...
        if pixel_values is not None:
            ...
            image_embeds, img_mask = self.visual(pixel_values, grid_thw=image_grid_thw)
