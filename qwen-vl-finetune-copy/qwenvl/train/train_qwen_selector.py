...
class LoggingTrainer(Trainer):
    """
    Simplified Trainer that logs task_loss and total_loss.
    No regularization weight scheduling needed with STE.
    """
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.is_world_process_zero() and self.state.global_step > 0 and self.state.global_step % self.args.logging_steps == 0:
            ...
            print(f"[Step {self.state.global_step}] total_loss: {total_loss:.5f}, task_loss: {task_loss:.5f}")
            try:
                import wandb
