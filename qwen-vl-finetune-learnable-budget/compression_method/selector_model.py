...
class TopK(Function):
    ...
    def backward(ctx, grad_output):
        ...
        # grad_k = (g^T v) / sum(v), derived from implicit differentiation of sum(sigma(x+t))=k
        grad_k = uv.sum(dim=1, keepdim=True) / s
        return t1 + uv, grad_k
...
def qwen25vl_vision_tower_forward_selector(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
    ...
    # Learnable budget: stored inside importance_scorer as nn.Parameter
    budget_value = self.importance_scorer.budget_value   # clamped to (0.01, 0.99)
    k_float = total_tokens * budget_value          # float k for differentiable topk (gradient flows)
    k_int = max(1, int(k_float.item()))            # int k for constraint loss (semantic correctness)

    img_mask = topk(learned_scores.unsqueeze(0), k_float).squeeze(0)
    ...
    with torch.no_grad():
        constraint_topk_indices = learned_scores.topk(k_int, dim=0).indices
    ...
    # -------------------------------------------------------------------------------
    return hidden_states_new, img_mask, constraint_img_mask, budget_value
...
def qwen25vl_generation_forward_selector(
    ...
    if inputs_embeds is None:
        ...
        if pixel_values is not None:
            ...
            image_embeds, img_mask, constraint_img_mask, budget_value = self.visual(pixel_values, grid_thw=image_grid_thw)
    ...
    if labels is not None:
        ...
        if pixel_values is not None:
            ...
            self.comp_loss = compression_loss.detach().item()
            loss += self.compression_weight * compression_loss

            # Log current budget value
            self.current_budget_value = budget_value.detach().item()
        # -------------------------------------------------------------------------------
